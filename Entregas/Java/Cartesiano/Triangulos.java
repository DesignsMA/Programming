...
class Triangulo {
    ...
    public Recta Base() { //Encuentra el lado mayor ( base ) y retorna la recta correspondiente 
        ...
        if (distancias[0] == Math.max( Math.max(distancias[0], distancias[1]), distancias[2] )) { //Si la recta de p1-p2 es la mayor de las otras
            return new Recta(p1, p2);
        } else if (distancias[1] == Math.max( Math.max(distancias[0], distancias[1]), distancias[2] )) { //Si no si la recta de p1-p3...
